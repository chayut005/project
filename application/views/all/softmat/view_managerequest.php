					...
					"render": function (data, type, row, meta) {
						...
						if (type === 'display') {
							...
							// data = '<button class="btn btn-primary" data-toggle="modal" data-id="' + data +'" data-title="' + data + '" data-book-id="' + data + '" data-target="#exampleModal" name = "myBtn" id ="myBtn" >'+ data +'</button>';               
								...
								if( row.status === '1' ){ 
                  html_data ='<button onclick="btncheck('+ data +')" class=" btn bg-success button1" style="color:white;background: linear-gradient(45deg, #75D247, #2ABF00 , #1F9B00);" ><i class="fa fa-check "></i></button>'
								}else{
								...
								html_data +='<button style="color:white;background: linear-gradient(45deg, #B8B8B8, #C3C3C3 , #9D9D9D);" class="btn bg-warning button1"><i class="fa fa-eye"></i></button>'
